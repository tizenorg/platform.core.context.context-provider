...
{
	if (strcmp(view_uri, _contacts_my_profile._uri) && strcmp(view_uri, _contacts_person._uri)) {
		_W("Unknown view uri");
		return;
	}

	std::string view = (!strcmp(view_uri, _contacts_my_profile._uri)? SOCIAL_ST_MY_PROFILE : SOCIAL_ST_PERSON);
...
bool ctx::social_status_contacts::is_consecutive_change(const char* view_uri)
{
	...
	if (!strcmp(view_uri, MY_PROFILE_VIEW)) {
		diff = difftime(now, latest_my_profile);
		latest_my_profile = now;
	} else if (!strcmp(view_uri, PERSON_VIEW)) {
...
bool ctx::social_status_contacts::set_callback()
{
	...
	IF_FAIL_RETURN_TAG(err == CONTACTS_ERROR_NONE, false, _E, "Setting my profile view changed callback failed");

	err = contacts_db_add_changed_cb(PERSON_VIEW, db_change_cb, this);
	IF_FAIL_RETURN_TAG(err == CONTACTS_ERROR_NONE, false, _E, "Setting person view changed callback failed");
