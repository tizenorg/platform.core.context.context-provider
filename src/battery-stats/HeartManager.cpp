...
int HeartManager::__executionCb(void *userData, int dim, char **value, char **column)
{
	...
	for (int i=0; i<dim; ++i) {
		...
		std::string temp(value[i]);
		row.set(NULL, column[i], temp);
	}
...
bool HeartManager::getAppData(std::string query, std::vector<Json>* app_vector)
{
	char *err = NULL;
	int ret;
	std:: string query2= query + "mik";
	_D ("[kunal] query is %s", query2.c_str());
	ret = sqlite3_exec(__databaseHandle, query.c_str(), __executionCb, app_vector, &err);
